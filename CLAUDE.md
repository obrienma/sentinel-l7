...
- **ADR-0034 guard doesn't sanitize `narrative`** — `guardPolicyRefsAgainstEmptyRetrieval()` only strips the structured `policy_refs` field; a fabricated citation embedded in the model's free-text `narrative` is untouched and still reaches `compliance_events.audit_narrative` (`AxiomProcessorService`) and the cached transaction analysis (`TransactionProcessorService`) — the actual persisted audit trail in both pipelines. Needs its own design pass (detecting/redacting an inline citation in prose is a different problem than clearing an array field).
- **ADR-0034 guard doesn't verify ref-to-chunk correspondence** — the guard only checks `chunk_count === 0`; when `chunk_count > 0` it does not verify a cited `policy_refs` entry actually matches a retrieved chunk. Closing this needs the retrieved chunk metadata to carry a stable citable identifier the model is prompted to echo verbatim — a prompt/schema change, not a pure guard-logic change.
