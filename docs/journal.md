...
## ADR-0034 Implementation — `policy_refs` Fabrication Guard on Empty Retrieval — 2026-08-01
...
`OpenRouterDriver`, `VertexAIDriver`) extend `AbstractComplianceDriver`
and none override `analyze()`, `analyzeTransaction()`, or
`parseResponse()`, one method in the shared base class covers all of
them: `guardPolicyRefsAgainstEmptyRetrieval()` forces `policy_refs = []`
...
when already empty), plus fixes to 4 existing tests across
`OllamaDriverTest`, `GeminiDriverTest`, and `OpenRouterDriverTest` whose
mocks had been accidentally exercising the fabrication path. Full suite:
396/396 passing, 0 regressions. Pint clean on all new/modified files
(pre-existing style findings in `GeminiDriverTest.php`/
`OpenRouterDriverTest.php` confirmed unrelated to this change via
`git stash`).
